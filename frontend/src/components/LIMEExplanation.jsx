...
export default function LIMEExplanation({ claimData = null, factors: apiFactors = null, loading: loadingProp = false }) {
  ...
  useEffect(() => {
    ...
    const loadFactors = () => {
      ...
      const amount = parseFloat(claimData.amount?.replace(/[$,]/g, '') || 5000);
      const age = parseInt(claimData.age, 10) || 35;
      const disease = claimData.disease || claimData.diagnosis || 'General';
    ...
    setTimeout(() => {
      if (isMounted) loadFactors();
    }, 250);

    return () => {
      isMounted = false;
    };
  }, [claimData, apiFactors]);
  ...
  if (loading) {
    return (
      <div className="mt-4">
        ...
        <p className="text-xs text-textSecondary mt-2">ML model analyzing claim patterns...</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-4">
      ...
      <div className="space-y-4">
        {factors.map((factor, index) => (
          <div key={index} className="relative">
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`text-xs font-mono ${
                    factor.color === 'high' ? 'text-danger' : factor.color === 'medium' ? 'text-warning' : 'text-success'
                  }`}
                >
                  {factor.impact}%
                </span>
            ...
            <div className="w-full bg-background rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all duration-500 ${
                  factor.color === 'high' ? 'bg-danger' : factor.color === 'medium' ? 'bg-warning' : 'bg-success'
                }`}
            ...
            {factor.description && (
              <p className="text-xs text-textSecondary mt-1 ml-1">{factor.description}</p>
            )}

            <div className="absolute -left-3 top-1/2 transform -translate-y-1/2">
              <div
                className={`w-1 h-8 rounded-full ${
                  factor.impact > 35 ? 'bg-danger' : factor.impact > 20 ? 'bg-warning' : 'bg-success'
                }`}
              ></div>
            </div>
          </div>
      ...
      <div className="mt-2 text-right">
        <span className="text-[10px] text-textSecondary">Analysis generated: {new Date().toLocaleString()}</span>
      </div>
    </div>
  );
}
